...
Redirect any emails to the administrator of the site.  This plugin will detect local installs (ending in .local or .dev) and WP engine staging installs (using is_wpe_snapshot() ).  You can also set a define to either force the behavior either on or off.

== Description ==

When using a local development install or staging site, for example WP Engine's staging [Staging site](http://support.wpengine.com/staging/), you may want to prevent the local or staging install of your site from sending emails to your site users. There are great server level tools for this like https://mailcatcher.me/ but often you may not have control over the server.  This plugin redirects all emails to the site admin.

The plugin will display a permenant admin notice when it's redirecting emails.
...
Note: There are many plugins out there that do interesting things to WP_Mail(). It is possible that one of those will interfer with or bypass this plugin. If you find an instance of that, please report it in the support forums so I can take a look.
...
Yes, but with one caveat. You can either activate this plugin on indivudal sites or network activate across the entire network, however it will only override emails sent by each indidivual site, not emails sent to the network admin (ex. new site creation).  For most this probably isn't an issue, but I welcome a pull request!
